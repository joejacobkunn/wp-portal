<div class="list-group overflow-scroll" style="height:600px">
    @if (!empty($this->orders) && $open_order_tab)
        @forelse ($this->orders->whereIn('stagecd',[1,2])->where('takenby', auth()->user()->sx_operator_id) as $order)
            ...
            <a wire:click="fetchOrderDetails({{ $order->orderno }},{{ $order->ordersuf }},'{{ $sro_number }}','open-order')"
                ...
                <div class="d-flex w-100 justify-content-between">
                    <h5 class="mb-1">Order# {{ $order->orderno }}-{{ $order->ordersuf }}
                        @if ($order->is_sro == 'SRO')
                            <span class="badge bg-light-danger"><i class="fas fa-tools"></i>
                                {{ $order->refer }}</span>
                        @endif
                    </h5>
                    <small><span class="badge bg-light-secondary">{{ $order->getStageCode() }}</span></small>
                </div>
                ...
                <small>
                    ...
                    <span class="badge bg-light-secondary">PROMISE DT :
                        {{ date(
                            "M j,
                                        Y",
                            strtotime($order->promisedt),
                        ) }}</span>
        ...
        @empty
            <div class="alert alert-light-warning color-warning">
                No open orders for this customer taken by me
            </div>
        @endforelse
    @else
