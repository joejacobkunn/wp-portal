<div>
    <div id="div-{{ $fieldDomId }}" class="x-multiselect">
        <select id="{{ $fieldDomId }}" name="{{ $fieldName }}" {{ $multiple ? 'multiple' : '' }}
            {{ $disabled ? 'disabled' : '' }}>
            @if ($selectAllOption)
    ...
    <script>
        ...
        function initPlugin() {
            ...
            function setData(values) {
                ...
                setTimeout(() => {
                    if (!document.querySelector('select[name={{ $fieldName }}]').closest('.x-multiselect')
                        .querySelector('.input-loading')) {
                        let loaderDom = document.createElement('i');
                        loaderDom.classList.add('fa', 'fa-spinner', 'fa-spin', 'input-loading')
                        document.querySelector('select[name={{ $fieldName }}]').closest('.x-multiselect')
                            .querySelector('.ss-main').appendChild(loaderDom)
                    }
                }, 200)
                ...
                selectTimer = setTimeout(() => {
                    document.dispatchEvent(new CustomEvent("browser:{{ $fieldId }}:changed", {
                        "detail": {
                            value: values
                        }
                    }));
            ...
            @if ($selectAllOption)
                ...
                rootElement.addEventListener('click', (event) => {
                    ...
                    if (targetElement.classList.contains('fa-times') || targetElement.classList.contains(
                            'ss-option')) {
                        setTimeout(() => {
                            initializeListeners();
                        }, 300)
                ...
                function initializeListeners() {
                    ...
                    if (selectField.data.searchValue) {
                        allFieldCount = selectField.data.data[0].options.filter((v) => v.text.includes(selectField.data
                            .searchValue)).length
                    }

                    let selectAllFlag = allFieldCount == selectField.data.data[0].options.filter((v) => v.selected).length
                    var element = document.getElementsByClassName(selectField.config.id)[0].getElementsByClassName(
                        'ss-optgroup-label')[0]
                    if (!selectAllFlag) {
    ...
    <script wire:ignore>
        ...
        function callbackListener(event) {
            if (document.querySelector('select[name={{ $fieldName }}]') && document.querySelector(
                    'select[name={{ $fieldName }}]').closest('.x-multiselect').querySelector('.input-loading')) {
                document.querySelector('select[name={{ $fieldName }}]').closest('.x-multiselect').querySelector(
