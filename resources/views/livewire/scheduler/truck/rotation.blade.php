        ...
        <div class="col-8 col-md-12 col-xxl-12">
            <div class="card border-light shadow-sm mb-4">
                ...
                <div class="card-body">
                    ...
                    @if(! $editRotation && !count($rotations))
                        <div class="mt-">
                            <p>No rotations are currently defined. <span>Click the link below to add a new rotation.</span></p>
                            <button
                                type="button"
                                wire:click="editRotationAction"
                                class="btn btn-outline-success mb-2">
                                <span
                                    wire:loading wire:target="editRotationAction"
                    ...
                    @else
                        ...
                        <div>
                                ...
                                @foreach($rotationData as $index => $rotaionRecord)
                                    <div class="rotation-ind-div" data-index="{{ $index }}">
                                        ...
                                        <div class="select-div">
                                            <x-forms.select
                                                model="rotationData.{{ $index}}"
                                                :options="$zones"
                                                :selected="$rotationData[$index] ?? ''"
                                                has-associative-index
                                                :key="$index . 'rotation' . time()"
                                            />
                                        </div>

                                        @if(count($rotationData) > 1 || !$loop->first)
                                        <button
                                            wire:click="removeRotationItem('{{ $index }}')"
                                            wire:confirm-action
                            ...
                            <button
                                type="button"
                                wire:click="addRotationItem"
                                class="btn btn-outline-primary my-2">
                                <span
                                    wire:loading wire:target="addRotationItem"
                                    class="spinner-border spinner-border-sm"
                                    role="status"
                                    aria-hidden="true"></span>
                            ...
                            <button
                                type="button"
                                wire:click="saveRotationData"
                                class="btn btn-success my-2">
                                <span
                                    wire:loading wire:target="saveRotationData"
                                    class="spinner-border spinner-border-sm"
                                    role="status"
                                    aria-hidden="true"></span>
                            ...
                            <button
                                type="button"
                                wire:click="cancelRotationData"
                                class="btn btn-outline-primary my-2">
                                <span
                                    wire:loading wire:target="cancelRotationData"
                                    class="spinner-border spinner-border-sm"
                                    role="status"
                                    aria-hidden="true"></span>
                                Cancel
                ...
                function initSortable() {
                    ...
                    $( ".rotation-form-div" ).sortable({
                        ...
                        stop: function( event, ui ) {
                            let order = [];
                            document.querySelectorAll('.rotation-ind-div').forEach((v) => { order.push(v.dataset.index) })
