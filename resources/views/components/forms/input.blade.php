<div class="form-group x-input">
    ...
    @if (!isset($noLabel))
        ...
        @if (isset($labelInfoText))
            <a tabindex="0" role="button" data-bs-toggle="popover" data-bs-placement="top" data-bs-trigger="focus"
    ...
    <div class="input-group">
        ...
        <input id="{{ $id }}" type="{{ $type ?? 'text' }}"
            ...
            placeholder="{{ $placeholder ?? '' }}" value="{{ $value ?? '' }}" {{ $isDisabled ? 'disabled' : '' }}
            wire:model.{{ !empty($defer) ? 'defer' : (!empty($live) ? 'live.debounce.500ms' : 'lazy') }}="{{ $model ?? '' }}"
            maxlength="{{ isset($maxLength) ? $maxLength : -1 }}" {!! !empty($enterAction) ? 'wire:keydown.enter="' . $enterAction . '"' : '' !!}
