...
<div x-data="{ open: false, childElementOpen: false, isTailwind: @js($component->isTailwind()), isBootstrap: @js($component->isBootstrap()) }" x-cloak x-show="(selectedItems.length > 0 || alwaysShowBulkActions)"
    @class([
        ...
        @else
            <div @class([
                ...
                @foreach ($component->getBulkActions() as $action => $title)
                    <a href="javascript:;"
                        @if ($component->hasConfirmationMessage($action)) wire:confirm="{{ $component->getBulkActionConfirmMessage($action) }}" @endif
                        wire:click="{{ $action }}" wire:key="{{ $tableName }}-bulk-action-{{ $action }}"
                        @class([
